...
param rM{D};
#param rM2;
param thetaE := pi/6; #Ángulo de salida
param thetaM := pi + pi/6; #Ángulo de entrada
...
subject to col_earth {n in NI}:
  sum{d in D} (s[d, n]-rE[d])^2 >= roE^2;

subject to col_moon {n in NI}:
  sum{d in D} (s[d, n]-rM[d])^2 >= roM^2;
